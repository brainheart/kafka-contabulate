"""Sanity checks on the generated Kafka build output."""
...
class TestWorks(unittest.TestCase):
    ...
    def test_genre_counts(self):
        ...
        self.assertEqual(counts['Novel'], 3)
        self.assertEqual(counts['Novella'], 3)
        self.assertEqual(counts['Story collection'], 2)
    ...
    def test_locations_follow_catalog_order(self):
        self.assertEqual(self.works[0]["location"], "01.BETR")
...
class TestParagraphs(unittest.TestCase):
    ...
    def test_paragraphs_are_present(self):
        self.assertEqual(len(self.paragraphs), 2201)

    def test_first_paragraph_shape(self):
        first = self.paragraphs[0]
        self.assertEqual(first["canonical_id"], "BETR.1.1")
        self.assertEqual(first["location"], "01.BETR.001.0001")
...
class TestTokens(unittest.TestCase):
    ...
    def test_kafka_vocabulary_present(self):
        for word in ('gesetz', 'schloss', 'ungeziefer', 'gericht'):
            self.assertIn(word, self.tokens)
        self.assertGreater(len(self.tokens["gesetz"]), 5)
...
class TestPublishedMetadata(unittest.TestCase):
    def test_hapax_counts_and_instance_json(self):
        ...
        self.assertEqual(instance["id"], "kafka")
        self.assertEqual(instance["created"], "2026-05-21")
        self.assertEqual(instance["stats"]["texts"], 11)
        self.assertEqual(instance["stats"]["segments"], 2201)
        self.assertEqual(instance["stats"]["words"], 333510)
    ...
    def test_characters_are_empty_for_kafka_build(self):
