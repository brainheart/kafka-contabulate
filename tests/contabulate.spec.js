...
test.describe('Page Load', () => {
  test('loads and shows the Kafka title', async ({ page }) => {
    await page.goto('/');
    await expect(page).toHaveTitle('Kafka Tabular Explorer');
  });
...
test.describe('Segments Search', () => {
  ...
  test('count cells drill down and ancestor cells filter', async ({ page }) => {
    await page.goto('/');
    await waitForDataLoaded(page);
    // Works → the 18 sections of the first work
    ...
    await expect(page.locator('#segmentsActiveFilters .active-filter-chip')).toContainText('starts with 01.BETR.');
    // Back un-drills
    await page.goBack();
    ...
    await expect(page.locator('#segmentsActiveFilters .active-filter-chip')).toContainText('is Story collection');
  });
  ...
  test('vocabulary granularities put n-grams in the rows', async ({ page }) => {
    ...
    await page.fill('#q', 'gesetz');
    await page.locator('#addColumnBtn').click();
    await expect(page.locator('#segmentsActiveFilters .active-filter-chip')).toContainText('matches gesetz');
    const words = await page.locator('#results tbody tr td:first-child').allTextContents();
    expect(words.length).toBeGreaterThan(0);
    expect(words.every(w => w.includes('gesetz'))).toBeTruthy();
  });
...
test('vocabulary scope survives switching the n-gram size', async ({ page }) => {
  await page.goto('/?gran=word&s_ft_location=%5E01%5C.BETR%5C.');
  await waitForDataLoaded(page);
  await page.waitForSelector('#results tbody tr', { timeout: 10000 });
  await expect(page.locator('#segmentsActiveFilters .active-filter-chip')).toContainText('starts with 01.BETR.');
